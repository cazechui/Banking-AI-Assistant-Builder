...
const ToolCallCard = ({ tool, onApprove }: { tool: ToolCallData, onApprove: () => void }) => (
  <div className="w-full max-w-[85%] mt-2 mb-2">
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 shadow-sm animate-fade-in-up">
      <div className="flex items-center gap-3 mb-3 border-b border-blue-200 pb-2">
        ...
        <div>
           <h4 className="font-bold text-blue-900 text-sm">Action Request</h4>
           <p className="text-xs text-blue-700">The agent wants to execute a function.</p>
        </div>
      </div>
      
      <div className="bg-white/60 rounded-lg p-3 font-mono text-xs text-blue-900 mb-3 overflow-x-auto">
      ...
      <button 
        onClick={onApprove}
        className="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold py-2 rounded-lg transition-colors flex items-center justify-center gap-2"
      >
...
const EscalationCard = ({ text, department }: { text: string; department: string }) => (
  <div className="w-full max-w-[85%] mt-2">
    ...
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3 shadow-sm animate-fade-in-up">
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-blue-800 mt-1">
           ...
           Ticket #{Math.floor(Math.random() * 10000)} created.
        </p>
        <div className="mt-3 flex gap-2">
           <div className="h-1.5 w-1.5 bg-blue-500 rounded-full animate-pulse"></div>
           <div className="h-1.5 w-1.5 bg-blue-500 rounded-full animate-pulse delay-100"></div>
           <div className="h-1.5 w-1.5 bg-blue-500 rounded-full animate-pulse delay-200"></div>
        </div>
      </div>
    </div>
  </div>
                ...
                <div 
                  className={`px-4 py-2.5 shadow-sm text-sm leading-relaxed whitespace-pre-wrap ${
                    msg.role === 'user' 
                      ...
                      : msg.role === 'system'
                        ? 'bg-slate-200/60 text-slate-500 text-xs py-1.5 px-3 rounded-lg mx-auto mb-2 text-center w-full italic'
                        : msg.isError 
      ...
      <div className="p-3 lg:p-4 bg-white border-t border-slate-200">
        <div className="relative flex items-center gap-2">
          <input 
            ...
            className={`flex-1 py-3 px-4 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-100 focus:bg-white transition text-sm disabled:opacity-80 ${
              !config.systemInstruction 
